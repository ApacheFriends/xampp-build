...
HTTPD_PIDFILE=@@XAMPP_APACHE_ROOTDIR@@/logs/httpd.pid


HTTPD="@@XAMPP_APACHE_ROOTDIR@@/bin/httpd -f @@XAMPP_APACHE_ROOTDIR@@/conf/httpd.conf"
...
. @@XAMPP_APACHE_ROOTDIR@@/bin/envvars
...
start_apache() {
    ...
    if [ $RUNNING -eq 1 ]; then
        ...
        cleanpid
        if $HTTPD ; then
            echo "$0 $ARG: httpd started at port @@XAMPP_APACHE_PORT@@"
        else
